...
function _empNome(login){
  const d=((login||'').split('@')[1]||'').toLowerCase();
  if(d.indexOf('nexxo')>=0)return'Nexxo Promotora';
  if(d.indexOf('novapromotora')>=0)return'Nova Promotora';
  return d||'—';
}
...
window._usoState={filtro:'todos',papel:'',sortCol:'dias',sortDir:1};

function _usoRender(){
  const tbl=document.getElementById('uso-tabela');
  if(!tbl||!window._usoDados)return;
  const dados=window._usoDados;
  const {filtro,papel,sortCol,sortDir}=window._usoState;
  const roleOrder={owner:0,admin:1,super:2,regional:3,comercial:4,gestor_convenios:5,pronto:6};
  const diasSem=function(u){return u.ultimo_acesso===null?9999:(u.dias_sem_acesso||0);};

  let vis=dados.filter(function(u){
    if(papel&&u.role!==papel)return false;
    const d=diasSem(u);
    if(filtro==='3d')return d>=3;
    if(filtro==='7d')return d>=7;
    if(filtro==='15d')return d>=15;
    return true;
  });
  ...
  vis=vis.slice().sort(function(a,b){
    ...
    else if(sortCol==='nome'){av=(a.nome||'').toLowerCase();bv=(b.nome||'').toLowerCase();}
    else if(sortCol==='login'){av=a.login||'';bv=b.login||'';}
    else if(sortCol==='dias'){av=diasSem(a);bv=diasSem(b);}
  ...
  const thS=function(col,lbl){
    const on=sortCol===col;
    const ico=on?(sortDir===1?' ▲':' ▼'):'';
    return '<th style="cursor:pointer;user-select:none'+(on?';color:#60a5fa':'')+'" onclick="_usoSort(\\''+col+'\\')">'+lbl+ico+'</th>';
  };
  const sel='background:#18182c;color:#e8e8f8;border:1px solid #252540;border-radius:8px;padding:6px 10px;font-size:11px';
  ...
  const lblFiltro={'todos':'Todos','3d':'3+ dias sem acesso','7d':'7+ dias sem acesso','15d':'15+ dias sem acesso'};

  let h='<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:14px">'
    +'<select onchange="window._usoState.filtro=this.value;_usoRender()" style="'+sel+'">'
    +'<option value="todos"'+(filtro==='todos'?' selected':'')+'>Todos</option>'
    +'<option value="3d"'+(filtro==='3d'?' selected':'')+'>3+ dias sem acesso</option>'
    +'<option value="7d"'+(filtro==='7d'?' selected':'')+'>7+ dias sem acesso</option>'
    +'<option value="15d"'+(filtro==='15d'?' selected':'')+'>15+ dias sem acesso</option>'
    +'</select>'
    +'<select onchange="window._usoState.papel=this.value;_usoRender()" style="'+sel+'">'
    ...
    +'</select>'
    +'<span style="font-size:11px;color:var(--muted2)">'+vis.length+' de '+dados.length+' usuário(s)'
    +(filtro!=='todos'?' &nbsp;<span style="color:#f59e0b;font-size:10px">— '+lblFiltro[filtro]+'</span>':'')
    +'</span>'
  ...
  h+='<div class="tbl-wrap"><table><thead><tr>'
    +thS('login','Login')+thS('nome','Nome')+thS('role','Papel')+'<th>Entidade</th>'
    +thS('ultimo','Último acesso')+thS('dias','Dias sem acesso')+thS('logins','Logins 30d')
    +'</tr></thead><tbody>';
  ...
  vis.forEach(function(u){
    ...
    const diasStr=d===9999?'nunca acessou':d===0?'hoje':d+'d';
    const diasColor=d===9999||d>=15?'color:#ef4444':d>=7?'color:#fb923c':d>=3?'color:#f59e0b':'color:#4ade80';
    const l30=u.logins_30d||0;
    const l30c=l30===0?'color:#6868a0':l30<5?'color:#9090c0':'color:#4ade80';
    h+='<tr>'
      ...
      +'<td style="font-size:12px;text-align:center;font-weight:700;'+diasColor+'">'+diasStr+'</td>'
      +'<td style="font-size:12px;text-align:center;'+l30c+'">'+l30+'</td>'
      +'</tr>';
  });
  h+='</tbody></table></div>'
...
async function _painelUsoLoad(){
  ...
  try{
    ...
    window._usoState={filtro:'todos',papel:'',sortCol:'dias',sortDir:1};
    const dias=function(u){return u.ultimo_acesso===null?9999:(u.dias_sem_acesso||0);};
    const total=dados.length;
    const d3=dados.filter(function(u){return dias(u)>=3;}).length;
    const d7=dados.filter(function(u){return dias(u)>=7;}).length;
    const d15=dados.filter(function(u){return dias(u)>=15;}).length;
    const kpiStyle='background:#18182c;border:1px solid #252540;border-radius:12px;padding:14px 20px;text-align:center;min-width:100px;cursor:pointer;transition:border .15s';
    const kpiVal='font-size:26px;font-weight:800';
    const kpiLbl='font-size:10px;color:#6868a0;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-top:4px';
    let kpis='<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">'
      +'<div style="'+kpiStyle+'" onclick="window._usoState.filtro=\\'todos\\';_usoRender()"><div style="'+kpiVal+';color:#e8e8f8">'+total+'</div><div style="'+kpiLbl+'">Total usuários</div></div>'
      +'<div style="'+kpiStyle+'" onclick="window._usoState.filtro=\\'3d\\';_usoRender()" title="Sem acessar há 3 dias ou mais"><div style="'+kpiVal+';color:#f59e0b">'+d3+'</div><div style="'+kpiLbl+'">3+ dias sem acesso</div></div>'
      +'<div style="'+kpiStyle+'" onclick="window._usoState.filtro=\\'7d\\';_usoRender()" title="Sem acessar há 7 dias ou mais"><div style="'+kpiVal+';color:#fb923c">'+d7+'</div><div style="'+kpiLbl+'">7+ dias sem acesso</div></div>'
      +'<div style="'+kpiStyle+'" onclick="window._usoState.filtro=\\'15d\\';_usoRender()" title="Sem acessar há 15 dias ou mais"><div style="'+kpiVal+';color:#ef4444">'+d15+'</div><div style="'+kpiLbl+'">15+ dias sem acesso</div></div>'
...
function _admRender(){
  const users=window._admUsers||[];
  const filtro=window._admFilter||'';
  const empresas=Array.from(new Set(users.map(u=>_empNome(u.login)))).sort();
  const vis=users.filter(u=>!filtro||_empNome(u.login)===filtro);
  let h='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;gap:10px;flex-wrap:wrap">'
    +'<span style="font-size:12px;color:var(--muted2)">'+vis.length+' de '+users.length+' usuário(s)</span>'
    +'<div style="display:flex;gap:8px;align-items:center">'
    +'<select onchange="window._admFilter=this.value;_admRender()" style="background:#18182c;color:#e8e8f8;border:1px solid #252540;border-radius:8px;padding:7px 10px;font-size:12px">'
    +'<option value="">Todas as empresas</option>'
    +empresas.map(function(e){return '<option value="'+e+'"'+(e===filtro?' selected':'')+'>'+e+'</option>';}).join('')
    +'</select>'
  ...
  vis.forEach(function(u){
    ...
    h+='<tr style="'+(u.bloqueado?'opacity:.5':'')+'">'
      ...
      +(isOwner?'':'<button class="exp-btn" onclick="_admAlterarEmail(\\''+u.login+'\\')">📧 Alt. Email</button> ')
  ...
  h+='</tbody></table></div>';
  document.getElementById('adm-box').innerHTML=h;
}
function prontoMapHas(u){return !!(u._pronto && u._pronto.indexOf('auto')<0);}
...
async function _admPronto(login,dar){
  const msg=dar?('Dar acesso de ADMIN da Pronto para '+login+'?\\n\\nEle vai gerenciar os usuários da Pronto dentro do app da Pronto.'):('Remover o acesso à Pronto de '+login+'?');
  if(!confirm(msg))return;
  try{await _adminRpc('pronto_set_papel',{alvo:login,p_papel:dar?'admin':null});initUsuariosTab();}
  catch(e){alert('Erro: '+e.message);}
}
...
function _admNovo(){
  ...
  $('nv-pronto').onchange=function(){$('nv-pronto-hint').style.display=$('nv-pronto').value==='admin'?'block':'none';};
  ...
  $('nv-ok').onclick=async()=>{
    ...
    const acessoPronto=$('nv-pronto').value;
    const email=$('nv-email').value.trim().toLowerCase();
    const senha=$('nv-senha').value;
    const nome=$('nv-nome').value.trim();
    const err=$('nv-err');
    try{
      if(role==='_pronto'){
        // Usuário só da Pronto (admin). Cria login + papel_pronto='admin', sem acesso à Gestão.
        await _adminRpc('pronto_criar_usuario',{p_email:email,p_senha:senha,p_nome:nome,p_papel:'admin'});
      }else{
        const ent=role==='super'?selSup.value:(role==='regional'?$('nv-reg').value:(role==='comercial'?$('nv-com').value:(role==='gestor_convenios'?email:'')));
      ...
      err.textContent='Erro: '+e.message;err.style.display='block';
    }
  };
}
...
function _admEditar(login){
  ...
  $('ed-ok').onclick=async()=>{
    ...
    const ent=role==='super'?selSup.value:(role==='regional'?$('ed-reg').value:(role==='comercial'?$('ed-com').value:(role==='gestor_convenios'?login:'')));
    const supE=(role==='regional'||role==='comercial')?selSup.value:'';
...
async function _admAlterarEmail(login){
  const iCss='width:100%;background:#18182c;color:#e8e8f8;border:1px solid #252540;border-radius:10px;padding:10px 12px;font-size:13px;margin-bottom:12px;box-sizing:border-box';
  const lCss='font-size:10px;color:#6868a0;font-weight:700;text-transform:uppercase;letter-spacing:.6px;display:block;margin-bottom:4px';
  let ov=document.createElement('div');
  ov.style.cssText='position:fixed;inset:0;background:rgba(5,5,15,.75);display:flex;align-items:center;justify-content:center;z-index:9999;font-family:Segoe UI,system-ui,sans-serif';
  ov.innerHTML='<div style="width:380px;background:#12121e;border:1px solid #252540;border-radius:16px;padding:26px 24px">'
    +'<div style="font-weight:700;color:#e8e8f8;font-size:15px;margin-bottom:6px">Alterar Email</div>'
    +'<div style="font-size:12px;color:var(--muted2);margin-bottom:16px">Email atual: <b>'+login+'</b></div>'
    +'<div id="ae-err" style="display:none;background:#ef444418;border:1px solid #ef444440;border-radius:8px;padding:8px 12px;font-size:12px;color:#ef4444;margin-bottom:12px;text-align:center"></div>'
    +'<label style="'+lCss+'">Novo email</label><input id="ae-email" type="email" style="'+iCss+'" placeholder="novo@novapromotora.com">'
    +'<label style="'+lCss+'">Confirmar novo email</label><input id="ae-email2" type="email" style="'+iCss+'" placeholder="novo@novapromotora.com">'
    +'<div style="display:flex;gap:8px;margin-top:4px">'
    +'<button id="ae-cancel" style="flex:1;padding:11px;border-radius:10px;border:1px solid #252540;background:none;color:#9090c0;font-size:13px;cursor:pointer">Cancelar</button>'
    +'<button id="ae-ok" style="flex:1;padding:11px;border-radius:10px;border:none;background:linear-gradient(135deg,#60a5fa,#818cf8);color:#fff;font-weight:700;font-size:13px;cursor:pointer">Salvar</button>'
    +'</div></div>';
  document.body.appendChild(ov);
  const $=id=>document.getElementById(id);
  $('ae-cancel').onclick=()=>ov.remove();
  $('ae-ok').onclick=async()=>{
    const novo=$('ae-email').value.trim().toLowerCase();
    const conf=$('ae-email2').value.trim().toLowerCase();
    const err=$('ae-err');
    if(!novo){err.textContent='Informe o novo email.';err.style.display='block';return;}
    if(novo!==conf){err.textContent='Os emails não coincidem.';err.style.display='block';return;}
    if(!/^[^@]+@[^@]+\.[^@]+$/.test(novo)){err.textContent='Email inválido.';err.style.display='block';return;}
    try{
      await _adminRpc('admin_alterar_email',{alvo:login,p_novo_email:novo});
      ov.remove();
      alert('Email alterado para '+novo+'. Informe o novo login para a pessoa.');
      initUsuariosTab();
